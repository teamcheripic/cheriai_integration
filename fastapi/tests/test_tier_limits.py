...
def test_all_tiers_present():
    assert set(CHERI_AI_DAILY_LIMITS) == {"basic", "premium-lite", "premium"}


def test_limits_increase_with_tier():
    assert (
        CHERI_AI_DAILY_LIMITS["basic"]
        < CHERI_AI_DAILY_LIMITS["premium-lite"]
        < CHERI_AI_DAILY_LIMITS["premium"]
    )


def test_price_map_targets_are_valid_tiers():
    for tier in STRIPE_PRICE_TO_TIER.values():
        assert tier in CHERI_AI_DAILY_LIMITS
